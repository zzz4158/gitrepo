...
export default class StForgetPassword extends Component{
	...
	handleSubmit = (e) => {
		...
		if(!stuId.length){
			this.setState({stuIdErrorMessage:'请输入学号'});
			return;
		}
		...
		HostPost('/forget-password',{stuId,email},false).then(({json})=>{
			if(json.error===0){
				...
				this.openNotification('error','您提交的邮箱与学号不对应，请再次检查！');
				this.setState({loading:false});
			}else if(json.error==='invalid_stuId') {
				this.openNotification('error','无效的学号，请再次检查！');
				this.setState({loading:false});
			}else {
		...
		return(
			<Form onSubmit={this.handleSubmit} className="login-form">
				...
				<FormItem validateStatus={stuIdErrorMessage.length ? 'error' : null} help={stuIdErrorMessage}>
					<Input prefix={<Icon type="user" />} placeholder="学号" value={stuId} onChange={this.handleChangeStuId}/>
				</FormItem>
				<FormItem validateStatus={emailErrorMessage.length ? 'error' : null} help={emailErrorMessage}>
					<Input prefix={<Icon type="mail" />}  placeholder="邮箱" value={email} onChange={this.handleChangeEmail}/>
				</FormItem>
				<FormItem>
					<Link to="/sign-in" className="login-form-forgot">返回登录</Link>
